# -*- coding: utf-8 -*-
"""
Created on Tue May 28 23:01:09 2024
...
#         self.features.fc = nn.Sequential()
        ...
        self._model.features[1].block[0][0] = nn.Conv2d(in_channels = _tmp2.in_channels, out_channels=_tmp1.out_channels,
            kernel_size= _tmp2.kernel_size, stride= 1, padding= _tmp2.padding, groups = _tmp2.groups, bias=use_bias)

        torch.cuda.empty_cache()
#         if(use_bias):
#             self._model.features[0][0].bias = _tmp.bias
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        k = self.key(x).view(B, T, self.n_head, self.dim_head).transpose(1, 2) # (B, nh, T, hs)
        q = self.query(x).view(B, T, self.n_head, self.dim_head).transpose(1, 2) # (B, nh, T, hs)
        v = self.value(x).view(B, T, self.n_head, self.dim_head).transpose(1, 2) # (B, nh, T, hs)
        ...
        y = y.transpose(1, 2).contiguous().view(B, T, self.n_head*self.dim_head) # re-assemble all head outputs side by side
...
class Encoder(nn.Module):
    ...
    def __init__(self, HSIband, lidarband, num_classes, config):
        super().__init__()
        self.config = config

        self.avgpool = nn.AdaptiveAvgPool2d((5, 5))
        
        self.image_encoder = ImageCNN(HSIband)
        self.lidar_encoder = LidarEncoder(lidarband)
        ...
        self.transformer1 = GPT(n_embd=16,
                            ...
                            config=config)
        self.transformer2 = GPT(n_embd=24,
                            n_head=config.n_head, 
                            dim_head = 24,
                            block_exp=config.block_exp, 
                            n_layer=config.n_layer, 
...
#         lidar_features = self.lidar_encoder._model.layer1(lidar_features)
        ...
        image_features_layer1 = F.interpolate(image_features_layer1, size=[11, 11], mode='bilinear')
        lidar_features_layer1 = F.interpolate(lidar_features_layer1, size=[11, 11], mode='bilinear')
...
#         lidar_embd_layer2 = self.avgpool(lidar_features)
        image_features_layer2, lidar_features_layer2 = self.transformer2(image_features2, lidar_features2)
...
#         lidar_features_layer2 = F.interpolate(lidar_features_layer2, scale_factor=4, mode='bilinear')
        image_features = image_features2 + image_features_layer2
        lidar_features = lidar_features2 + lidar_features_layer2
