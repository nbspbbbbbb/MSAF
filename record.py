...
def record_output(oa_ae, aa_ae, kappa_ae, element_acc_ae,train,test,infer, macs, params, train_sample, path):
    f = open(path, 'w')
    sentence_1 = 'training samples are:' + str(train_sample) + '\n'
    f.write(sentence_1)
    ...
    sentence11 += "\n" + '\n'
    f.write(sentence11)
    sentence12 = 'train time for each iteration are:' + str(train) + '\n' + '\n'
    f.write(sentence12)
    sentence13 = 'test time for each iteration are:' + str(test) + '\n' + '\n'
    f.write(sentence13) 
    sentence13_1 = 'infer all pixels time for each iteration are:' + str(infer) + '\n' + '\n'
    f.write(sentence13_1) 
    
    sentence14 = 'all iters acc:' + str(element_acc_ae) + '\n' + '\n'
    f.write(sentence14) 
    sentence15 = 'The models parameters and Macs are:' + str(params) + str(macs) + '\n' + '\n'
    f.write(sentence15)
